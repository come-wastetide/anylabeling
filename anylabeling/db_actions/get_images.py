...
def get_scan(scan_id: str):
    response = supabase.table("Scan").select().eq("id", scan_id).execute().json()
...
def download_unreviewed_scans(limit_date=None,organisation_id=None):
    ...
    if limit_date:
        ...
        scans_response = supabase.table("Scan").select().in_("id", scan_ids).execute().model_dump_json()
    return json.loads(scans_response)

def get_picture_url(scan_id: str):
    response = supabase.table("Scan").select("pictureUrl").eq("id", scan_id).execute().json()
    return json.loads(response)
...
def download_picture(scan_id: str, destination = ''):
    ...
    if not destination.endswith(f'{scan_id}.jpg'):
        destination = f'{destination}/{scan_id}.jpg' if destination else f'./{scan_id}.jpg'
...
def download_data_line_json(scan_id: str, destination = ''):
    ...
    if not destination.endswith(f'{scan_id}.json'):
        destination = f'{destination}/{scan_id}.json' if destination else f'./{scan_id}.json'
